Shader "MingShader/SingleTexture（Blinn-Phong）" {
	Properties{
		...
		struct v2f {
			...
			float3 worldPosition : TEXCOORD1;	//世界坐标下的顶点
		...
		v2f vert(a2v v) {
			...
			//f.uv = TRANSFORM_TEX(v.texcoord, _BodyTex);
			f.uv = v.texcoord.xy * _BodyTex_ST.xy + _BodyTex_ST.zw;
			return f;
		}
		...
		fixed4 frag(v2f f) :SV_Target{
			//材质的反射率 （纹素值 Texel）
			fixed3 albedo = tex2D(_BodyTex,f.uv).rgb * _BodyColor.rgb;
			...
			//-------------------------
			//漫反射颜色
			fixed3 diffuse = _LightColor0.rgb * albedo * (0.5f * (dot(worldNormal, lightDirection)) + 0.5f);	 //_LightColor0第一个直射光的颜色
			//-------------------------
